...
class getRedirect(BaseEstimator, TransformerMixin):
    def fit(self, X, y=None):
        return self
    def transform(self, X, y=None):
        print('hashtag')
        tx = np.array([url.count('?q=') for url in X['url']]).reshape(-1, 1)
        return tx
...
class getExpire(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        print('check complete whois')
        return np.array([url for url in X['expire']]).reshape(-1, 1)

class getUpdate(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        print('check complete whois')
        return np.array([url for url in X['update']]).reshape(-1, 1)
...
class getSSL_train(BaseEstimator, TransformerMixin):
    def fit(self, X, y=None):
        return self
    def transform(self, X, y=None):
        tx = np.array([SSLstate_train(url) for url in X['url']]).reshape(-1, 1)
        return tx
